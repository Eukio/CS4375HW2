...
class CrackerBox(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if base.endswith('-box.txt'):
            image_name = base.replace('-box.txt', '.jpg')
        else:
        ...
        tensor_image = torch.from_numpy(image_resized.transpose(2, 0, 1)).float()
        pixel_mean_tensor = torch.from_numpy(self.pixel_mean.reshape(3, 1, 1)).float()
        image_blob = (tensor_image - pixel_mean_tensor) / 255.0
        ...
        grid_x = max(0, min(self.yolo_grid_num - 1, grid_x))
        grid_y = max(0, min(self.yolo_grid_num - 1, grid_y))
        offset_x = (cx - grid_x * self.yolo_grid_size) / self.yolo_grid_size
        offset_y = (cy - grid_y * self.yolo_grid_size) / self.yolo_grid_size
        w_norm = bw / self.yolo_image_size
        h_norm = bh / self.yolo_image_size

        tensor_gt_box = torch.zeros(5, self.yolo_grid_num, self.yolo_grid_num)
        tensor_gt_box[0, grid_y, grid_x] = offset_x
        tensor_gt_box[1, grid_y, grid_x] = offset_y
        tensor_gt_box[2, grid_y, grid_x] = w_norm
        tensor_gt_box[3, grid_y, grid_x] = h_norm
        tensor_gt_box[4, grid_y, grid_x] = 1.0
        gt_box_blob = tensor_gt_box.float()

        tensor_gt_mask = torch.zeros(self.yolo_grid_num, self.yolo_grid_num)
        tensor_gt_mask[grid_y, grid_x] = 1.0
        gt_mask_blob = tensor_gt_mask.float()
