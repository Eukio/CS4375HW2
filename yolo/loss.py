...
def compute_loss(output, pred_box, gt_box, gt_mask, num_boxes, num_classes, grid_size, image_size):
    ...
    pred_confidence = output[:,4:5*num_boxes:5, :, :]

    gt_x = gt_box[:,0].unsqueeze(1)
    gt_y = gt_box[:,1].unsqueeze(1)
    gt_w = gt_box[:,2].unsqueeze(1)
    gt_h = gt_box[:,3].unsqueeze(1)

    loss_x = weight_coord * torch.sum( box_mask * (pred_x - gt_x)**2 )
    loss_y = weight_coord * torch.sum( box_mask * (pred_y - gt_y)**2 )
    loss_w = weight_coord * torch.sum( box_mask * (torch.sqrt(pred_w.clamp(min=1e-6)) - torch.sqrt(gt_w))**2)
    loss_h = weight_coord * torch.sum( box_mask * (torch.sqrt(pred_h.clamp(min=1e-6)) - torch.sqrt(gt_h))**2)

    loss_obj = torch.sum( box_mask * (pred_confidence - box_confidence)**2 )
    loss_noobj = (weight_noobj) * torch.sum((1-box_mask) * (pred_confidence-0)**2)

    pred_cls = output[:, 5*num_boxes : 5*num_boxes + num_classes]
    gt_cls = gt_box[:, 4].unsqueeze(1)
    loss_cls = torch.sum(gt_mask.unsqueeze(1) * (pred_cls - gt_cls)**2)
